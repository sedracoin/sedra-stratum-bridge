...
import (
	...
	"github.com/sedracoin/Sedra-stratum-bridge/src/sedrastratum"
	"gopkg.in/yaml.v2"
)
...
func main() {
	...
	flag.StringVar(&cfg.RPCServer, "sedra", cfg.RPCServer, "address of the sedrad node, default `localhost:22110`")
	...
	if cfg.BlockWaitTime == 0 {
		cfg.BlockWaitTime = 5 * time.Second // this should never happen due to sdr 1s block times
	}

	log.Println("----------------------------------")
	log.Printf("initializing bridge")
	log.Printf("\tsedrad:          %s", cfg.RPCServer)
