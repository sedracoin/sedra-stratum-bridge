...
func ListenAndServe(cfg BridgeConfig) error {
	...
	sdApi, err := NewSedraAPI(cfg.RPCServer, blockWaitTime, logger)
	if err != nil {
		return err
	}
	...
	shareHandler := newShareHandler(sdApi.sedrad)
	...
	defer cancel()
	sdApi.Start(ctx, func() {
		clientHandler.NewBlockAvailable(sdApi)
	})
