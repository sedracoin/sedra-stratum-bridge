...
func (c *clientListener) NewBlockAvailable(sedi *SedraApi) {
	c.clientLock.Lock()
	...
	for _, cl := range c.clients {
		...
		go func(client *gostratum.StratumContext) {
			...
			template, err := sedi.GetBlockTemplate(client)
			if err != nil {
	...
	if time.Since(c.lastBalanceCheck) > balanceDelay {
		...
		if len(addresses) > 0 {
			go func() {
				balances, err := sedi.sedrad.GetBalancesByAddresses(addresses)
				if err != nil {
